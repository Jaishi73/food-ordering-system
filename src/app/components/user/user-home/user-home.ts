...
openChef(chef: Chef): void {
    // for now just log or alert – later replace with router navigation
 console.log('Navigating to chef:', chef); 
    this.router.navigate(['/chef', chef.id]);
    //  alert(`Clicked Chef ${chef.name} (ID: ${chef.id})`);

//  // TEMPORARY: Show chef details in alert + console
//   alert(`Chef ${chef.name}\n${chef.speciality}\n⭐ ${chef.rating}`);
//   console.log('Chef selected:', chef);
  
//   // Simulate navigation by updating a property
//   this.selectedChef = chef;  // Add this property to show chef info
  }

  goToCheckout(): void {
  
  //     console.log('🛒 CLICK DETECTED - goToCheckout()');
  // alert('Navigating...');  // Popup confirms
